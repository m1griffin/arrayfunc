...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		result = arrayfunc.aall('>', datax, compval)
	endtime = time.perf_counter()
...
def BenchmarkAFErrTrueSimdTrue(afitercounts, arraycode, arraydata, optiondata):
	...
	starttime = time.perf_counter()
	for i in range(afitercounts):
		result = arrayfunc.aall('>', datax, compval, nosimd=True)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "result = arrayfunc.aall('>', datax, compval, nosimd=True)"

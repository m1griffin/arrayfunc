...
def BenchmarkAFErrTrueSimdTrue(afitercounts, arraycode, arraydata, optiondata):
	...
	starttime = time.perf_counter()
	for i in range(afitercounts):
		arrayfunc.lshift(datax, yvalue, dataout, nosimd=True)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "arrayfunc.lshift(datax, yvalue, dataout, nosimd=True)"

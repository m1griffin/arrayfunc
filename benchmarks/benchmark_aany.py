...
def InitOptionData(arraycode, arraysize, funcname):
	...
	if arraycode in ('f', 'd'):
		optiondata.compval =  float(50)
	else:
		optiondata.compval =  int(50)
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		result = arrayfunc.aany('>', datax, compval)
	endtime = time.perf_counter()
...
def BenchmarkAFErrTrueSimdTrue(afitercounts, arraycode, arraydata, optiondata):
	...
	starttime = time.perf_counter()
	for i in range(afitercounts):
		result = arrayfunc.aany('>', datax, compval, nosimd=True)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "result = arrayfunc.aany('>', datax, compval, nosimd=True)"

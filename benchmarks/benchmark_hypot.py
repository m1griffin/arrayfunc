	...
	if True:
		for x in range(pyitercounts):
			for i in range(arraylength):
				dataout[i] = math.hypot(datax[i], yvalue)
	else:
		for x in range(pyitercounts):
			dataout[i] = math.hypot(datax[i], yvalue)
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		arrayfunc.hypot(datax, yvalue, dataout)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "arrayfunc.hypot(datax, yvalue, dataout, matherrors=True)"

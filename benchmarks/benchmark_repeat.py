	...
	if False:
		for x in range(pyitercounts):
			for i in range(arraylength):
				datax = array.array(arraycode, itertools.repeat(compval, arraysize))
	else:
		for x in range(pyitercounts):
			datax = array.array(arraycode, itertools.repeat(compval, arraysize))
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		arrayfunc.repeat(datax, compval)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "arrayfunc.repeat(datax, compval)"

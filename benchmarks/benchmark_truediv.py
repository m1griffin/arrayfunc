	...
	if True:
		for x in range(pyitercounts):
			for i in range(arraylength):
				dataout[i] = truediv_type(datax[i] / yvalue)
	else:
		for x in range(pyitercounts):
			dataout[i] = truediv_type(datax[i] / yvalue)
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		arrayfunc.truediv(datax, yvalue, dataout)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "arrayfunc.truediv(datax, yvalue, dataout, matherrors=True)"

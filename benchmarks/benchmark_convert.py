	...
	if False:
		for x in range(pyitercounts):
			for i in range(arraylength):
				dataout = array.array("d", [float(x) for x in datax])
	else:
		for x in range(pyitercounts):
			dataout = array.array("d", [float(x) for x in datax])
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		arrayfunc.convert(datax, dataout)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "arrayfunc.convert(datax, dataout)"

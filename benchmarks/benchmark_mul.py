...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		arrayfunc.mul(datax, yvalue, dataout)
	endtime = time.perf_counter()
...
def BenchmarkAFErrFalseSimdTrue(afitercounts, arraycode, arraydata, optiondata):
	...
	starttime = time.perf_counter()
	for i in range(afitercounts):
		arrayfunc.mul(datax, yvalue, dataout, nosimd=True)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "arrayfunc.mul(datax, yvalue, dataout, matherrors=True, nosimd=True)"

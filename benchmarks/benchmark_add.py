...
def BenchmarkAFErrFalseSimdTrue(afitercounts, arraycode, arraydata, optiondata):
	...
	starttime = time.perf_counter()
	for i in range(afitercounts):
		arrayfunc.add(datax, yvalue, dataout, nosimd=True)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "arrayfunc.add(datax, yvalue, dataout, matherrors=True, nosimd=True)"

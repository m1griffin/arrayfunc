	...
	if False:
		for x in range(pyitercounts):
			for i in range(arraylength):
				dataout = array.array(arraycode, itertools.compress(datax, pycomp))
	else:
		for x in range(pyitercounts):
			dataout = array.array(arraycode, itertools.compress(datax, pycomp))
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		result = arrayfunc.compress(pycomp, dataout, compdata)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "result = arrayfunc.compress(pycomp, dataout, compdata)"

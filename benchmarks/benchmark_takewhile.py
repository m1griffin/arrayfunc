	...
	if False:
		for x in range(pyitercounts):
			for i in range(arraylength):
				dataout = array.array(arraycode, itertools.takewhile(lambda x : x < compval, datax))
	else:
		for x in range(pyitercounts):
			dataout = array.array(arraycode, itertools.takewhile(lambda x : x < compval, datax))
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		result = arrayfunc.takewhile('<', datax, dataout, compval)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "result = arrayfunc.takewhile('<', datax, dataout, compval)"

...
##############################################################################
# Project:  arrayfunc
# Module:   benchmark_findindices.py
...
def InitOptionData(arraycode, arraysize, funcname):
	...
	if arraycode in ('f', 'd'):
		optiondata.compval =  float(10)
	else:
		optiondata.compval =  int(10)
	...
	if False:
		for x in range(pyitercounts):
			for i in range(arraylength):
				result = pyfindindices(datax, fidataout, compval)
	else:
		for x in range(pyitercounts):
			result = pyfindindices(datax, fidataout, compval)
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		result = arrayfunc.findindices('==', datax, fidataout, compval)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "result = arrayfunc.findindices('==', datax, fidataout, compval)"

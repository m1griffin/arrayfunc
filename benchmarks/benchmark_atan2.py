	...
	if True:
		for x in range(pyitercounts):
			for i in range(arraylength):
				dataout[i] = math.atan2(datax[i], yvalue)
	else:
		for x in range(pyitercounts):
			dataout[i] = math.atan2(datax[i], yvalue)
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		arrayfunc.atan2(datax, yvalue, dataout)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "arrayfunc.atan2(datax, yvalue, dataout, matherrors=True)"

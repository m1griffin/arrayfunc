...
def InitOptionData(arraycode, arraysize, funcname):
	...
	if arraycode in ('f', 'd'):
		optiondata.compval =  float(5)
	else:
		optiondata.compval =  int(5)
	...
	if False:
		for x in range(pyitercounts):
			for i in range(arraylength):
				result = array.array(arraycode, filter(lambda x: x < compval, datax))
	else:
		for x in range(pyitercounts):
			result = array.array(arraycode, filter(lambda x: x < compval, datax))
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		result = arrayfunc.afilter('<', datax, dataout, compval)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "result = arrayfunc.afilter('<', datax, dataout, compval)"

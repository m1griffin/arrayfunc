	...
	if False:
		for x in range(pyitercounts):
			for i in range(arraylength):
				result = pyfindindex(datax, compval)
	else:
		for x in range(pyitercounts):
			result = pyfindindex(datax, compval)
...
def BenchmarkAF(afitercounts, arraycode, arraydata, optiondata):
	...
	for i in range(afitercounts):
		result = arrayfunc.findindex('==', datax, compval)
	endtime = time.perf_counter()
...
def BenchmarkAFErrTrueSimdTrue(afitercounts, arraycode, arraydata, optiondata):
	...
	starttime = time.perf_counter()
	for i in range(afitercounts):
		result = arrayfunc.findindex('==', datax, compval, nosimd=True)
	endtime = time.perf_counter()
...
HasMathErrorOption = 'matherrors' in "result = arrayfunc.findindex('==', datax, compval, nosimd=True)"

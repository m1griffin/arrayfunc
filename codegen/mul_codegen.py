...
##############################################################################
# Project:  arrayfunc
# Purpose:  Generate the C code for math multiple operations. 
#			parameter.
# Language: Python 3.8
# Date:     30-Dec-2017
#
		...
		// Math error checking enabled.
			...
			if (param == 0) {
				...
				if (param == -1) {
					...
					ovlimit1 = max_ovlimit_%(funcmodifier)s(param);
					ovlimit2 = min_ovlimit_%(funcmodifier)s(param);
...
%(simd_call_5)s
	...
	// Math error checking enabled.
		for (x = 0; x < arraylen; x++) {
			if ((data2[x] > 0) && pos_loop_willoverflow_%(funcmodifier)s(data1[x], data2[x]) ) {return ARR_ERR_OVFL;}
			if ((data2[x] < -1) && neg_loop_willoverflow_%(funcmodifier)s(data1[x], data2[x]) ) {return ARR_ERR_OVFL;}
			if ((data2[x] == -1) && minusone_loop_willoverflow_%(funcmodifier)s(data1[x]) ) {return ARR_ERR_OVFL;}
			data1[x] = data1[x] * data2[x];
		}
	}
	return ARR_NO_ERR;
...
%(simd_call_1_ovfl)s
		...
		// Math error checking enabled.
			if (param == 0) {
				...
				ovlimit = uint_ovlimit_%(funcmodifier)s(param);
				for (x = 0; x < arraylen; x++) {
...
%(simd_call_5)s
	...
	// Math error checking enabled.
		for (x = 0; x < arraylen; x++) {
			if ( uint_loop_willoverflow_%(funcmodifier)s(data1[x], data2[x]) ) {return ARR_ERR_OVFL;}
			data1[x] = data1[x] * data2[x];
		}
	}
	return ARR_NO_ERR;
...
#define max_ovlimit_%(funcmodifier)s(val) %(intmaxvalue)s / val
#define min_ovlimit_%(funcmodifier)s(val) %(intminvalue)s / val
...
#define pos_loop_willoverflow_%(funcmodifier)s(lval, rval) ((lval > (%(intmaxvalue)s / rval)) || (lval < (%(intminvalue)s / rval)))
...
#define uint_ovlimit_%(funcmodifier)s(val) %(intmaxvalue)s / val
// For unsigned integer loops.
#define uint_loop_willoverflow_%(funcmodifier)s(lval, rval) ((rval != 0) && (lval > (%(intmaxvalue)s / rval)))
...
simd_ovcheck_armv71 = ''' 
			// Check for overflow. 
			if (!(%(vreinterpinstr)s(ovcheck1) == 0x0000000000000000)){'''

simd_ovcheck_armv72 = ''' 
			// Check for overflow. 
			if ((!(%(vreinterpinstr)s(ovcheck1) == 0x0000000000000000)) ||
				(!(%(vreinterpinstr)s(ovcheck2) == 0x0000000000000000))){'''


simd_pos_willoverflow_12_armv7 = '''// Check for overflow.
			ovcheck1 = %(vgtinstr)s (datasliceleft, ovflvec1);
			ovcheck2 = %(vltinstr)s (datasliceleft, ovflvec2);''' + simd_ovcheck_armv72

simd_pos_willoverflow_34_armv7 = '''// Check for overflow.
			ovcheck1 = %(vgtinstr)s (datasliceright, ovflvec1);
			ovcheck2 = %(vltinstr)s (datasliceright, ovflvec2);''' + simd_ovcheck_armv72


simd_neg_willoverflow_12_armv7 = '''// Check for overflow.
			ovcheck1 = %(vltinstr)s (datasliceleft, ovflvec1);
			ovcheck2 = %(vgtinstr)s (datasliceleft, ovflvec2);''' + simd_ovcheck_armv72

simd_neg_willoverflow_34_armv7 = '''// Check for overflow.
			ovcheck1 = %(vltinstr)s (datasliceright, ovflvec1);
			ovcheck2 = %(vgtinstr)s (datasliceright, ovflvec2);''' + simd_ovcheck_armv72


simd_equ_willoverflow_12_armv7 = '''// Do an equal compare operation.
			ovcheck1 = %(veqinstr)s (datasliceleft, ovflvec1);''' + simd_ovcheck_armv71

simd_equ_willoverflow_34_armv7 = '''// Do an equal compare operation.
			ovcheck1 = %(veqinstr)s (datasliceright, ovflvec1);''' + simd_ovcheck_armv71


simd_unsigned_willoverflow_12_armv7 = '''// Check for overflow.
		ovcheck1 = %(vgtinstr)s (datasliceleft, ovflvec);''' + simd_ovcheck_armv71

simd_unsigned_willoverflow_34_armv7 = '''// Check for overflow.
		ovcheck1 = %(vgtinstr)s (datasliceright, ovflvec);''' + simd_ovcheck_armv71
...
simd_ovcheck_armv81 = ''' 
			...
			veccombine = %(vreinterpinstr)s(ovcheck1);
...
simd_ovcheck_armv82 = ''' 
			...
			// Combine the result to two 64 bit vectors.
			veccombine = %(vreinterpinstr)s(ovcheck2);
			// Get the high and low lanes of the combined vector.
...
simd_pos_willoverflow_12_armv8 = '''// Check for overflow.
			ovcheck1 = %(vgtinstr)s (datasliceleft, ovflvec1);
			ovcheck2 = %(vltinstr)s (datasliceleft, ovflvec2);''' + simd_ovcheck_armv82

simd_pos_willoverflow_34_armv8 = '''// Check for overflow.
			ovcheck1 = %(vgtinstr)s (datasliceright, ovflvec1);
			ovcheck2 = %(vltinstr)s (datasliceright, ovflvec2);''' + simd_ovcheck_armv82


simd_neg_willoverflow_12_armv8 = '''// Check for overflow.
			ovcheck1 = %(vltinstr)s (datasliceleft, ovflvec1);
			ovcheck2 = %(vgtinstr)s (datasliceleft, ovflvec2);''' + simd_ovcheck_armv82

simd_neg_willoverflow_34_armv8 = '''// Check for overflow.
			ovcheck1 = %(vltinstr)s (datasliceright, ovflvec1);
			ovcheck2 = %(vgtinstr)s (datasliceright, ovflvec2);''' + simd_ovcheck_armv82


simd_equ_willoverflow_12_armv8 = '''// Do an equal compare operation.
			ovcheck1 = %(veqinstr)s (datasliceleft, ovflvec1);''' + simd_ovcheck_armv81

simd_equ_willoverflow_34_armv8 = '''// Do an equal compare operation.
			ovcheck1 = %(veqinstr)s (datasliceright, ovflvec1);''' + simd_ovcheck_armv81


simd_unsigned_willoverflow_12_armv8 = '''// Check for overflow.
			ovcheck1 = %(vgtinstr)s (datasliceleft, ovflvec);''' + simd_ovcheck_armv81

simd_unsigned_willoverflow_34_armv8 = '''// Check for overflow.
			ovcheck1 = %(vgtinstr)s (datasliceright, ovflvec);''' + simd_ovcheck_armv81


simd_ovflchk_extravars_armv8_int = '''uint64x2_t veccombine;
	uint64_t highresult1, lowresult1, highresult2, lowresult2;'''

simd_ovflchk_extravars_armv8_uint = '''uint64x2_t veccombine;
...
char %(funclabel)s_%(funcmodifier)s_1_simd_ovfl(Py_ssize_t arraylen, %(arraytype)s *data1, %(arraytype)s param) {
	...
	%(arraytype)s ovlimit1, ovlimit2;
	%(simdattr)s datasliceleft, datasliceright, resultslice, ovflvec1, ovflvec2;
	%(ovflsimdattr)s ovcheck1, ovcheck2;
	%(simd_ovflchk_extravars_int)s
	...
	if (param == -1) {
		...
		ovflvec1 = initvec_%(funcmodifier)s(%(intminvalue)s);
		...
		for (x = alignedlength; x < arraylen; x++) {
			if ( minusone_loop_willoverflow_%(funcmodifier)s(data1[x]) ) {return 1;}
			data1[x] = data1[x] * param; 
		}
		...
		// This is used for detecting a potential overflow condition.
		ovflvec1 = initvec_%(funcmodifier)s(ovlimit1);
		ovflvec2 = initvec_%(funcmodifier)s(ovlimit2);
...
char %(funclabel)s_%(funcmodifier)s_3_simd_ovfl(Py_ssize_t arraylen, %(arraytype)s param, %(arraytype)s *data2) {
	...
	if (param == -1) {
		...
		// Handle the values left over at the end of the array.
		for (x = alignedlength; x < arraylen; x++) {
			if ( minusone_loop_willoverflow_%(funcmodifier)s(data2[x]) ) {return 1;}
...
char %(funclabel)s_%(funcmodifier)s_1_simd_ovfl(Py_ssize_t arraylen, %(arraytype)s *data1, %(arraytype)s param) {
	...
	%(ovflsimdattr)s ovcheck1;
	%(simd_ovflchk_extravars_uint)s
	...
	for (x = 0; x < alignedlength; x += %(simdwidth)s) {
		...
		// Check for overflow. 
		%(simd_unsigned_willoverflow_12)s
			return 1;
		}
...
char %(funclabel)s_%(funcmodifier)s_3_simd_ovfl(Py_ssize_t arraylen, %(arraytype)s param, %(arraytype)s *data2) {
	...
	for (x = 0; x < alignedlength; x += %(simdwidth)s) {
		...
		// Check for overflow. 
		%(simd_unsigned_willoverflow_34)s
			return 1;
		}
...
def findsimdplatform(arraycode, funcname):
	...
	if hasarmv7 and hasarmv8:
		return SIMD_platform_ARM
	elif (not hasarmv7) and hasarmv8:
...
uint_template = ops_mul_uint
int_template = ops_mul_int


includeoptions = includeoptions_arm
...
def CreateHeader(funcname):
	...
	funcdata = {'funclabel' : funcname, 
				'includeoptions' : includeoptions % {'funclabel' : funcname},
				}
...
def CreateArrayDataCCode(arraycode, funcname):
	...
	if arraycode in (set(armv7_simdtypes) | set(armv8_simdtypes)):
...
def SetSIMDData_ARMv7(funcname):
	...
	for arraycode in armv7_simdtypes:
		...
		funcdata = {'funclabel' : funcname,
					...
					'simd_ovflchk_extravars_int' : '',
					'simd_ovflchk_extravars_uint' : '',
		...
		if arraycode in simdovfl_armv7:
			...
			funcdata['simd_equ_willoverflow_12'] = simd_equ_willoverflow_12_armv7 % simddata
			funcdata['simd_equ_willoverflow_34'] = simd_equ_willoverflow_34_armv7 % simddata
			funcdata['simd_unsigned_willoverflow_12'] = simd_unsigned_willoverflow_12_armv7 % simddata
			funcdata['simd_unsigned_willoverflow_34'] = simd_unsigned_willoverflow_34_armv7 % simddata
...
def SetSIMDData_ARMv8(funcname):
	...
	for arraycode in armv8_simdtypes:
		...
		funcdata = {'funclabel' : funcname,
					...
					'simd_ovflchk_extravars_int' : simd_ovflchk_extravars_armv8_int,
					'simd_ovflchk_extravars_uint' : simd_ovflchk_extravars_armv8_uint,
		...
		if arraycode in simdovfl_armv8:
			...
			funcdata['simd_equ_willoverflow_12'] = simd_equ_willoverflow_12_armv8 % simddata
			funcdata['simd_equ_willoverflow_34'] = simd_equ_willoverflow_34_armv8 % simddata
			funcdata['simd_unsigned_willoverflow_12'] = simd_unsigned_willoverflow_12_armv8 % simddata
			funcdata['simd_unsigned_willoverflow_34'] = simd_unsigned_willoverflow_34_armv8 % simddata

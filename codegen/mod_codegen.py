...
##############################################################################
# Project:  arrayfunc
# Purpose:  Generate the C code for math mod operations. 
#			parameter.
# Language: Python 3.8
# Date:     30-Dec-2017
#
...
signed int %(funclabel)s_%(funcmodifier)s_5(Py_ssize_t arraylen, %(arraytype)s *data1, %(arraytype)s *data2, unsigned int ignoreerrors) {
	...
	for (x = 0; x < arraylen; x++) {
		...
		// This requires a special case to avoid integer overflow.
		if ((data2tmp == -1) && (datatmp == %(intminvalue)s)) {
			data1[x] = 0;
		} else {
...
float_template = ops_mod_float
uint_template = ops_mod_uint
int_template = ops_mod_int

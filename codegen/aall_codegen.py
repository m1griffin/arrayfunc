...
##############################################################################
# Project:  arrayfunc
# Purpose:  Generate the C code for aall.
# Language: Python 3.4
# Date:     11-Jun-2014
#
###############################################################################
...
*/
signed int aall_%(funcmodifier)s(signed int opcode, Py_ssize_t arraylen, %(arraytype)s *data, %(arraytype)s param1) { 
...
/* For array code: %(arraycode)s
   ...
   Returns 1 if the condition was true for all array elements, ARR_ERR_NOTFOUND
		 if it was false at least once, or an error code if the opcode was invalid.
*/
signed int aall_%(funcmodifier)s(signed int opcode, Py_ssize_t arraylen, %(arraytype)s *data, %(arraytype)s param1, unsigned int nosimd) { 
	...
	if (!nosimd && (arraylen >= (%(simdwidth)s * 2))) {
		return aall_%(funcmodifier)s_simd(opcode, arraylen, data, param1);
	}
#endif
...
*/
#ifdef AF_HASSIMD
signed int aall_%(funcmodifier)s_simd(signed int opcode, Py_ssize_t arraylen, %(arraytype)s *data, %(arraytype)s param1) { 
...
# For floating point.
simdcomp_f = {
	'==' : '!(__builtin_ia32_pmovmskb128((v16qi) resultslice) == 0xffff)',
	'>' : '!(__builtin_ia32_pmovmskb128((v16qi) resultslice) == 0xffff)',
	'>=' : '!(__builtin_ia32_pmovmskb128((v16qi) resultslice) == 0xffff)',
	'<' : '!(__builtin_ia32_pmovmskb128((v16qi) resultslice) == 0xffff)',
	'<=' : '!(__builtin_ia32_pmovmskb128((v16qi) resultslice) == 0xffff)',

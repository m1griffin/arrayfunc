...
##############################################################################
class trunc_general_f(unittest.TestCase):
	...
	def test_trunc_outputarray(self):
		"""Test trunc to output array - Array code f.
		"""
		...
		expected = [math.trunc(x) for x in data]
		arrayfunc.trunc(data, dataout)
	...
	########################################################
	def test_trunc_outputarray_ov(self):
		"""Test trunc to output array with matherrors=True  - Array code f.
		"""
		data = array.array('f', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
		...
		expected = [math.trunc(x) for x in data]
		arrayfunc.trunc(data, dataout, matherrors=True)
	...
	########################################################
	def test_trunc_outputarray_lim(self):
		"""Test trunc to output array with array limit  - Array code f.
		"""
		data = array.array('f', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
		...
		pydataout = [math.trunc(x) for x in data]
		expected = pydataout[0:limited] + list(dataout)[limited:]

		arrayfunc.trunc(data, dataout, maxlen=limited)
	...
	########################################################
	def test_trunc_outputarray_ov_lim(self):
		"""Test trunc to output array with matherrors=True and array limit - Array code f.
		"""
		data = array.array('f', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
		...
		arrayfunc.trunc(data, dataout, matherrors=True, maxlen=limited)
	...
	def test_trunc_inplace(self):
		"""Test trunc in place - Array code f.
		"""
		data = array.array('f', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])


		expected = [math.trunc(x) for x in data]
		arrayfunc.trunc(data)
	...
	########################################################
	def test_trunc_inplace_ov(self):
		"""Test trunc in place with matherrors=True  - Array code f.
		"""
		data = array.array('f', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])


		expected = [math.trunc(x) for x in data]
		arrayfunc.trunc(data, matherrors=True)
	...
	########################################################
	def test_trunc_inplace_lim(self):
		"""Test trunc in place with array limit  - Array code f.
		"""
		data = array.array('f', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
		...
		arrayfunc.trunc(data, maxlen=limited)
	...
	########################################################
	def test_trunc_inplace_ov_lim(self):
		"""Test trunc in place with matherrors=True and array limit  - Array code f.
		"""
		data = array.array('f', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
		...
		arrayfunc.trunc(data, matherrors=True, maxlen=limited)
...
class trunc_param_errors_f(unittest.TestCase):
	...
	def test_trunc_array_none_b1(self):
		"""Test trunc as *array-none* for errors='a' - Array code f.
		"""
		...
		# This is the actual test.
		with self.assertRaises(TypeError):
			arrayfunc.trunc(floatarray, errors='a')
	...
	def test_trunc_array_num_array_d1(self):
		"""Test trunc as *array-num-array* for errors='a' - Array code f.
		"""
		# This version is expected to pass.
		...
		# This is the actual test.
		with self.assertRaises(TypeError):
			arrayfunc.trunc(self.floatarray, self.dataout, errors='a')
...
##############################################################################
class trunc_general_d(unittest.TestCase):
	"""Test for basic general function operation.
	test_template_noparams
	"""
	...
	########################################################
	def test_trunc_outputarray(self):
		"""Test trunc to output array - Array code d.
		"""
	...
	########################################################
	def test_trunc_outputarray_ov(self):
		"""Test trunc to output array with matherrors=True  - Array code d.
		"""
		data = array.array('d', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
	...
	########################################################
	def test_trunc_outputarray_lim(self):
		"""Test trunc to output array with array limit  - Array code d.
		"""
		data = array.array('d', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
	...
	########################################################
	def test_trunc_outputarray_ov_lim(self):
		"""Test trunc to output array with matherrors=True and array limit - Array code d.
		"""
		data = array.array('d', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
	...
	########################################################
	def test_trunc_inplace(self):
		"""Test trunc in place - Array code d.
		"""
		data = array.array('d', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
	...
	########################################################
	def test_trunc_inplace_ov(self):
		"""Test trunc in place with matherrors=True  - Array code d.
		"""
		data = array.array('d', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
	...
	########################################################
	def test_trunc_inplace_lim(self):
		"""Test trunc in place with array limit  - Array code d.
		"""
		data = array.array('d', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
	...
	########################################################
	def test_trunc_inplace_ov_lim(self):
		"""Test trunc in place with matherrors=True and array limit  - Array code d.
		"""
		data = array.array('d', [0.0,2.4,4.8,7.2,9.6,12.0,14.4,16.8,19.2,21.6])
...
class trunc_param_errors_d(unittest.TestCase):
	...
	########################################################
	def test_trunc_array_none_b1(self):
		"""Test trunc as *array-none* for errors='a' - Array code d.
		"""
		# Copy the array so we don't change the original data.
		floatarray = copy.copy(self.floatarray)
	...
	########################################################
	def test_trunc_array_num_array_d1(self):
		"""Test trunc as *array-num-array* for errors='a' - Array code d.
		"""
		# This version is expected to pass.
		arrayfunc.trunc(self.floatarray, self.dataout, matherrors=True)

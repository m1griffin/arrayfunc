...
void sub_signed_char_1_simd(Py_ssize_t arraylen, signed char *data1, signed char param) {
	...
	for (x = 0; x < alignedlength; x += CHARSIMDSIZE) {
		...
		resultslice = vsubq_s8(datasliceleft, datasliceright);
		// Store the result.
...
char sub_signed_char_3_simd_ovfl(Py_ssize_t arraylen, signed char param, signed char *data2) {
	...
	if (param == 0) {
		...
		for (x = 0; x < alignedlength; x += CHARSIMDSIZE) {
			...
			ovcheck = vceqq_s8 (datasliceright, ovflvec); 
			...
			resultslice = vnegq_s8(datasliceright);
	...
	if (param > 0) {
		...
		for (x = 0; x < alignedlength; x += CHARSIMDSIZE) {
			...
			// Check for overflow. 
			// Do a less than compare operation.
			ovcheck = vcltq_s8 (datasliceright, ovflvec); 
	...
	if (param < 0) {
		...
		for (x = 0; x < alignedlength; x += CHARSIMDSIZE) {
			...
			// Check for overflow. 
			// Do a greater than compare operation.
			ovcheck = vcgtq_s8 (datasliceright, ovflvec); 
...
void sub_unsigned_char_1_simd(Py_ssize_t arraylen, unsigned char *data1, unsigned char param) {
	...
	for (x = 0; x < alignedlength; x += CHARSIMDSIZE) {
		...
		// The actual SIMD operation. 
		resultslice = vsubq_u8(datasliceleft, datasliceright);
		// Store the result.
...
char sub_unsigned_char_1_simd_ovfl(Py_ssize_t arraylen, unsigned char *data1, unsigned char param) {
	...
	for (x = 0; x < alignedlength; x += CHARSIMDSIZE) {
		...
		// Check for overflow. 
		// Do a less than compare operation.
			ovcheck = vcltq_u8 (datasliceleft, datasliceright); 
...
void sub_signed_short_1_simd(Py_ssize_t arraylen, signed short *data1, signed short param) {
	...
	for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
		...
		// The actual SIMD operation. 
		resultslice = vsubq_s16(datasliceleft, datasliceright);
		// Store the result.
...
char sub_signed_short_3_simd_ovfl(Py_ssize_t arraylen, signed short param, signed short *data2) {
	...
	if (param == 0) {
		...
		for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
			...
			// Check for overflow. 
			// Do an equal compare operation.
			ovcheck = vceqq_s16 (datasliceright, ovflvec); 
			...
			// The actual SIMD operation. Since we are subtracting from
			// zero we simply negate it.
			resultslice = vnegq_s16(datasliceright);
	...
	if (param > 0) {
		...
		for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
			...
			// Check for overflow. 
			// Do a less than compare operation.
			ovcheck = vcltq_s16 (datasliceright, ovflvec); 
	...
	if (param < 0) {
		...
		for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
			...
			// Check for overflow. 
			// Do a greater than compare operation.
			ovcheck = vcgtq_s16 (datasliceright, ovflvec); 
...
void sub_unsigned_short_1_simd(Py_ssize_t arraylen, unsigned short *data1, unsigned short param) {
	...
	for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
		...
		// The actual SIMD operation. 
		resultslice = vsubq_u16(datasliceleft, datasliceright);
		// Store the result.
...
char sub_unsigned_short_1_simd_ovfl(Py_ssize_t arraylen, unsigned short *data1, unsigned short param) {
	...
	for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
		...
		// Check for overflow. 
		// Do a less than compare operation.
			ovcheck = vcltq_u16 (datasliceleft, datasliceright); 
...
void sub_signed_int_1_simd(Py_ssize_t arraylen, signed int *data1, signed int param) {
	...
	for (x = 0; x < alignedlength; x += INTSIMDSIZE) {
		...
		// The actual SIMD operation. 
		resultslice = vsubq_s32(datasliceleft, datasliceright);
		// Store the result.
...
char sub_signed_int_3_simd_ovfl(Py_ssize_t arraylen, signed int param, signed int *data2) {
	...
	if (param == 0) {
		...
		for (x = 0; x < alignedlength; x += INTSIMDSIZE) {
			...
			// Check for overflow. 
			// Do an equal compare operation.
			ovcheck = vceqq_s32 (datasliceright, ovflvec); 
			...
			// The actual SIMD operation. Since we are subtracting from
			// zero we simply negate it.
			resultslice = vnegq_s32(datasliceright);
	...
	if (param > 0) {
		...
		for (x = 0; x < alignedlength; x += INTSIMDSIZE) {
			...
			// Check for overflow. 
			// Do a less than compare operation.
			ovcheck = vcltq_s32 (datasliceright, ovflvec); 
	...
	if (param < 0) {
		...
		for (x = 0; x < alignedlength; x += INTSIMDSIZE) {
			...
			// Check for overflow. 
			// Do a greater than compare operation.
			ovcheck = vcgtq_s32 (datasliceright, ovflvec); 
...
// param_arr_num_none
#if defined(AF_HASSIMD_ARM_AARCH64)
void sub_unsigned_int_1_simd(Py_ssize_t arraylen, unsigned int *data1, unsigned int param) {
	...
	for (x = 0; x < alignedlength; x += INTSIMDSIZE) {
		...
		// The actual SIMD operation. 
		resultslice = vsubq_u32(datasliceleft, datasliceright);
		// Store the result.
...
// param_arr_num_arr
void sub_unsigned_int_2_simd(Py_ssize_t arraylen, unsigned int *data1, unsigned int param, unsigned int *data3) {
...
// param_num_arr_none
void sub_unsigned_int_3_simd(Py_ssize_t arraylen, unsigned int param, unsigned int *data2) {
...
// param_num_arr_arr
void sub_unsigned_int_4_simd(Py_ssize_t arraylen, unsigned int param, unsigned int *data2, unsigned int *data3) {
...
// param_arr_arr_none
void sub_unsigned_int_5_simd(Py_ssize_t arraylen, unsigned int *data1, unsigned int *data2) {
...
// param_arr_arr_arr
void sub_unsigned_int_6_simd(Py_ssize_t arraylen, unsigned int *data1, unsigned int *data2, unsigned int *data3) {
...
// param_arr_num_none
#if defined(AF_HASSIMD_ARM_AARCH64)
char sub_unsigned_int_1_simd_ovfl(Py_ssize_t arraylen, unsigned int *data1, unsigned int param) {
	...
	for (x = 0; x < alignedlength; x += INTSIMDSIZE) {
		...
		// Check for overflow. 
		// Do a less than compare operation.
			ovcheck = vcltq_u32 (datasliceleft, datasliceright); 
	...
	// Handle the values left over at the end of the array.
	for (x = alignedlength; x < arraylen; x++) {
		if ( loop_willoverflow_unsigned_int(data1[x], param) ) {return 1;}
		data1[x] = data1[x] - param;
	}
...
// param_arr_num_arr
char sub_unsigned_int_2_simd_ovfl(Py_ssize_t arraylen, unsigned int *data1, unsigned int param, unsigned int *data3) {
...
// param_num_arr_none
char sub_unsigned_int_3_simd_ovfl(Py_ssize_t arraylen, unsigned int param, unsigned int *data2) {
	...
	// Handle the values left over at the end of the array.
	for (x = alignedlength; x < arraylen; x++) {
		if ( loop_willoverflow_unsigned_int(param, data2[x]) ) {return 1;}
		data2[x] = param - data2[x];
	}
...
// param_num_arr_arr
char sub_unsigned_int_4_simd_ovfl(Py_ssize_t arraylen, unsigned int param, unsigned int *data2, unsigned int *data3) {
...
// param_arr_arr_none
char sub_unsigned_int_5_simd_ovfl(Py_ssize_t arraylen, unsigned int *data1, unsigned int *data2) {
...
// param_arr_arr_arr
char sub_unsigned_int_6_simd_ovfl(Py_ssize_t arraylen, unsigned int *data1, unsigned int *data2, unsigned int *data3) {
...
void sub_float_1_simd(Py_ssize_t arraylen, float *data1, float param) {
	...
	for (x = 0; x < alignedlength; x += FLOATSIMDSIZE) {
		...
		// The actual SIMD operation. 
		resultslice = vsubq_f32(datasliceleft, datasliceright);
		// Store the result.
...
char sub_float_1_simd_ovfl(Py_ssize_t arraylen, float *data1, float param) {
	...
	float32x4_t datasliceleft, datasliceright, resultslice, checkslice;
	...
	for (x = 0; x < alignedlength; x += FLOATSIMDSIZE) {
		...
		checkslice = vmulq_f32(checkslice, resultslice);
	}

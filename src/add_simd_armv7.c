...
char add_unsigned_char_1_simd_ovfl(Py_ssize_t arraylen, unsigned char *data1, unsigned char param) {
	...
	for (x = 0; x < alignedlength; x += CHARSIMDSIZE) {
		...
		// Check for overflow. 
		// Do a greater than compare operation.
			ovcheck = vcgt_u8 (datasliceleft, ovflvec);
...
char add_unsigned_char_5_simd_ovfl(Py_ssize_t arraylen, unsigned char *data1, unsigned char *data2) {
	...
	static unsigned char ovcompvals[CHARSIMDSIZE] = {UCHAR_MAX, UCHAR_MAX, UCHAR_MAX, UCHAR_MAX, UCHAR_MAX, UCHAR_MAX, UCHAR_MAX, UCHAR_MAX};
	uint8x8_t datasliceleft, datasliceright, resultslice, ovcompslice, ovflvec;
	uint8x8_t ovcheck;
	...
	ovcompslice = vld1_u8( ovcompvals);
	...
	for (x = 0; x < alignedlength; x += CHARSIMDSIZE) {
		...
		ovflvec = vsub_u8(ovcompslice, datasliceright);
...
char add_signed_short_3_simd_ovfl(Py_ssize_t arraylen, signed short param, signed short *data2) {
	...
	if (param > 0) {
		...
		for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
			// Load the data into the vector register.
			datasliceleft = vld1_s16( &data2[x]);
...
char add_unsigned_short_1_simd_ovfl(Py_ssize_t arraylen, unsigned short *data1, unsigned short param) {
	...
	for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
		...
		// Check for overflow. 
		// Do a greater than compare operation.
			ovcheck = vcgt_u16 (datasliceleft, ovflvec);
...
char add_unsigned_short_3_simd_ovfl(Py_ssize_t arraylen, unsigned short param, unsigned short *data2) {
	...
	for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
		// Load the data into the vector register.
		datasliceleft = vld1_u16( &data2[x]);
...
char add_unsigned_short_5_simd_ovfl(Py_ssize_t arraylen, unsigned short *data1, unsigned short *data2) {
	...
	static unsigned short ovcompvals[SHORTSIMDSIZE] = {USHRT_MAX, USHRT_MAX, USHRT_MAX, USHRT_MAX};
	uint16x4_t datasliceleft, datasliceright, resultslice, ovcompslice, ovflvec;
	uint16x4_t ovcheck;
	...
	// Used to check for overflows.
	ovcompslice = vld1_u16( ovcompvals);
	...
	for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
		...
		// Subtract the right hand value.
		ovflvec = vsub_u16(ovcompslice, datasliceright);
...
*/
#if defined(AF_HASSIMD_ARMv7_32BIT)
float32x2_t initvec_float(float initval) {
...
void add_float_1_simd(Py_ssize_t arraylen, float *data1, float param) {
	...
	float32x2_t datasliceleft, datasliceright, resultslice;
	...
	for (x = 0; x < alignedlength; x += FLOATSIMDSIZE) {
		// Load the data into the vector register.
		datasliceleft = vld1_f32( &data1[x]);
		// The actual SIMD operation. 
		resultslice = vadd_f32(datasliceleft, datasliceright);
		// Store the result.
		vst1_f32( &data1[x],  resultslice);
	}
...
void add_float_3_simd(Py_ssize_t arraylen, float param, float *data2) {
	...
	for (x = 0; x < alignedlength; x += FLOATSIMDSIZE) {
		// Load the data into the vector register.
		datasliceright = vld1_f32( &data2[x]);
		// The actual SIMD operation.
		resultslice = vadd_f32(datasliceleft, datasliceright);
		// Store the result.
		vst1_f32( &data2[x],  resultslice);
	}
...
// param_arr_num_none
#if defined(AF_HASSIMD_ARMv7_32BIT)
char add_float_1_simd_ovfl(Py_ssize_t arraylen, float *data1, float param) {
	...
	float32x2_t datasliceleft, datasliceright, resultslice, checkslice;
	...
	for (x = 0; x < alignedlength; x += FLOATSIMDSIZE) {
		...
		checkslice = vmul_f32(checkslice, resultslice);
	}
...
// param_arr_num_arr
char add_float_2_simd_ovfl(Py_ssize_t arraylen, float *data1, float param, float *data3) {
...
// param_num_arr_none
char add_float_3_simd_ovfl(Py_ssize_t arraylen, float param, float *data2) {
...
// param_num_arr_arr
char add_float_4_simd_ovfl(Py_ssize_t arraylen, float param, float *data2, float *data3) {
...
// param_arr_arr_none
char add_float_5_simd_ovfl(Py_ssize_t arraylen, float *data1, float *data2) {
...
// param_arr_arr_arr
char add_float_6_simd_ovfl(Py_ssize_t arraylen, float *data1, float *data2, float *data3) {

...
struct args_param parsepyargs_parm(PyObject *args, PyObject *keywds) {
	...
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "UOOO|n:dropwhile", kwlist, 
...
static PyObject *py_dropwhile(PyObject *self, PyObject *args, PyObject *keywds) {
	...
	switch (itemcode) {
		...
		case 'b' : {
			...
			if (!PyArg_ParseTupleAndKeywords(args, keywds, "Uy*y*l|n:dropwhile", kwlist, 
		...
		case 'h' : {
			// The format string and parameter names depend on the expected data types.
			if (!PyArg_ParseTupleAndKeywords(args, keywds, "Uy*y*h|n:dropwhile", kwlist, 
		...
		case 'i' : {
			// The format string and parameter names depend on the expected data types.
			if (!PyArg_ParseTupleAndKeywords(args, keywds, "Uy*y*i|n:dropwhile", kwlist, 
		...
		case 'L' : {
			...
			if (!PyArg_ParseTupleAndKeywords(args, keywds, "Uy*y*k|n:dropwhile", kwlist, 
					&opstr, &datapy, &dataoutpy, &param1py.L, &arraymaxlen)) {
				return NULL;
			}
			break;
		...
		case 'f' : {
			// The format string and parameter names depend on the expected data types.
			if (!PyArg_ParseTupleAndKeywords(args, keywds, "Uy*y*f|n:dropwhile", kwlist, 
		...
		case 'd' : {
			// The format string and parameter names depend on the expected data types.
			if (!PyArg_ParseTupleAndKeywords(args, keywds, "Uy*y*d|n:dropwhile", kwlist, 
	...
	switch(itemcode) {
		// signed char
		case 'b' : {
			resultcode = dropwhile_signed_char(opcode, arraylength, data.b, dataout.b, param1py.b);
			break;
		}
		// unsigned char
		case 'B' : {
			resultcode = dropwhile_unsigned_char(opcode, arraylength, data.B, dataout.B, param1py.B);
			break;
		}
		// signed short
		case 'h' : {
			resultcode = dropwhile_signed_short(opcode, arraylength, data.h, dataout.h, param1py.h);
			break;
		}
		// unsigned short
		case 'H' : {
			resultcode = dropwhile_unsigned_short(opcode, arraylength, data.H, dataout.H, param1py.H);
			break;
		}
		// signed int
		case 'i' : {
			resultcode = dropwhile_signed_int(opcode, arraylength, data.i, dataout.i, param1py.i);
			break;
		}
		// unsigned int
		case 'I' : {
			resultcode = dropwhile_unsigned_int(opcode, arraylength, data.I, dataout.I, param1py.I);
			break;
		}
		// signed long
		case 'l' : {
			resultcode = dropwhile_signed_long(opcode, arraylength, data.l, dataout.l, param1py.l);
			break;
		}
		// unsigned long
		case 'L' : {
		...
		// double
		case 'd' : {
			resultcode = dropwhile_double(opcode, arraylength, data.d, dataout.d, param1py.d);
			break;
		}
		// We don't know this code.
		default: {

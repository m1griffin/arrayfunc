...
static char *kwlist_2[] = {"data1", "data2", "dataout", "maxlen", NULL};
 ...
 * Returns: Nothing.
*/
void releasebuffers_noerr2(struct args_params_noerr2 arraydata) {
...
*/
struct args_params_noerr2 getparams_noerr2(PyObject *self, PyObject *args, PyObject *keywds, char *funcname) {
	...
	switch(prmclass){
		case CAT_INT_ARR_NUM : {
			if (ischeckedintcode(arraytype)) {
				...
				makefmtstr("y*L|y*n:", funcname, formatstr);

				// Import the parsed parameters. 
				if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &datapy1, &paramtmp_q, &datapy3, &arraymaxlen)) {
					ErrMsgParameterError();
					arraydata.error = 5;
					return arraydata;
				}
				...
				switch (arraytype) {
					case 'L': {
						makefmtstr("y*k|y*n:", funcname, formatstr);

						// Import the parsed parameters. 
						if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &datapy1, &parampy.L, &datapy3, &arraymaxlen)) {
							ErrMsgParameterError();
							arraydata.error = 5;
							return arraydata;
						}
					...
					case 'Q':{
						...
						// Import the parsed parameters. 
						if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &datapy1, &parampy.Q, &datapy3, &arraymaxlen)) {
							ErrMsgParameterError();
							arraydata.error = 5;
							return arraydata;
						}
		...
		case CAT_FLOAT_ARR_NUM : {
			makefmtstr("y*f|y*n:", funcname, formatstr);

			// Import the parsed parameters. 
			if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &datapy1, &parampy.f, &datapy3, &arraymaxlen)) {
				ErrMsgParameterError();
				arraydata.error = 5;
				return arraydata;
			}
		...
		case CAT_DOUBLE_ARR_NUM : {
			makefmtstr("y*d|y*n:", funcname, formatstr);

			// Import the parsed parameters. 
			if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &datapy1, &parampy.d, &datapy3, &arraymaxlen)) {
				ErrMsgParameterError();
				arraydata.error = 5;
				return arraydata;
			}
		...
		case CAT_INT_NUM_ARR : {
			if (ischeckedintcode(arraytype)) {
				...
				// manually, so we check almost all integers this way. For 
				// parameter format strings, "L" is for a signed long long int.
				makefmtstr("Ly*|y*n:", funcname, formatstr);

				// Import the parsed parameters. 
				if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &paramtmp_q, &datapy2, &datapy3, &arraymaxlen)) {
					ErrMsgParameterError();
					arraydata.error = 5;
					return arraydata;
				}
				...
				switch (arraytype) {
					case 'L': {
						makefmtstr("ky*|y*n:", funcname, formatstr);

						// Import the parsed parameters. 
						if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &parampy.L, &datapy2, &datapy3, &arraymaxlen)) {
							ErrMsgParameterError();
							arraydata.error = 5;
							return arraydata;
						}
					...
					case 'Q':{
						...
						// Import the parsed parameters. 
						if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &parampy.Q, &datapy2, &datapy3, &arraymaxlen)) {
							ErrMsgParameterError();
							arraydata.error = 5;
							return arraydata;
						}
		...
		case CAT_FLOAT_NUM_ARR : {
			makefmtstr("fy*|y*n:", funcname, formatstr);

			// Import the parsed parameters. 
			if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &parampy.f, &datapy2, &datapy3, &arraymaxlen)) {
				ErrMsgParameterError();
				arraydata.error = 5;
				return arraydata;
			}
		...
		case CAT_DOUBLE_NUM_ARR : {
			makefmtstr("dy*|y*n:", funcname, formatstr);

			// Import the parsed parameters. 
			if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &parampy.d, &datapy2, &datapy3, &arraymaxlen)) {
				ErrMsgParameterError();
				arraydata.error = 5;
				return arraydata;
			}
		...
		case CAT_ANY_ARR_ARR : {
			makefmtstr("y*y*|y*n:", funcname, formatstr);

			// Import the parsed parameters. 
			if (!PyArg_ParseTupleAndKeywords(args, keywds, formatstr, kwlist_2, &datapy1, &datapy2, &datapy3, &arraymaxlen)) {
				ErrMsgParameterError();
				arraydata.error = 5;
				return arraydata;
			}

	...
	while(dataindex < arraylen) {
		...
		while(opindex < codearraylen) {
			...
			switch (codearray[opindex]) {
				...
				// math.log2
				case CALCOP_FLOAT_MATH_LOG2: {
					// Overflow checking disabled.
					if (disableovfl) {
						for(x = 0; x < slicestride; x++) {
							vmstack[stackpointer + x] = log2f(vmstack[stackpointer + x]);
						}
					} else {
						for(x = 0; x < slicestride; x++) {
							vmstack[stackpointer + x] = log2f(vmstack[stackpointer + x]);
		...
		while(opindex < codearraylen) {
			...
			switch (codearray[opindex]) {
				...
				case CALCOP_FLOAT_MATH_LOG2: {
					...
					if (disableovfl) {
						for(x = 0; x < slicestride; x++) {
							vmstack[stackpointer + x] = log2(vmstack[stackpointer + x]);
						}
					} else {
						for(x = 0; x < slicestride; x++) {
							vmstack[stackpointer + x] = log2(vmstack[stackpointer + x]);

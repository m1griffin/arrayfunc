...
long long innerloop_asum_signed_char(Py_ssize_t arraylen, signed char *data) { 
	...
	int8x16_t dataslice;
	int16x8_t resultslice;


	// Initialise the accumulator.
	resultslice = vdupq_n_s16(0);
	...
	for (x = 0; x < alignedlength; x += CHARSIMDSIZE) {
		...
		resultslice = vpadalq_s8(resultslice, dataslice);

	}

	// Add up the values within the slice.
	vst1q_s16(sumvals, resultslice);
...
long long innerloop_asum_signed_short(Py_ssize_t arraylen, signed short *data) { 
	...
	int16x8_t dataslice;
	int32x4_t resultslice;


	// Initialise the accumulator.
	resultslice = vdupq_n_s32(0);
	...
	for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
		...
		// The actual SIMD operation. 
		resultslice = vpadalq_s16(resultslice, dataslice);

	}

	// Add up the values within the slice.
	vst1q_s32(sumvals, resultslice);
...
unsigned long long innerloop_asum_unsigned_short(Py_ssize_t arraylen, unsigned short *data) { 
	...
	uint16x8_t dataslice;
	uint32x4_t resultslice;


	// Initialise the accumulator.
	resultslice = vdupq_n_u32(0);
	...
	for (x = 0; x < alignedlength; x += SHORTSIMDSIZE) {
		...
		// The actual SIMD operation. 
		resultslice = vpadalq_u16(resultslice, dataslice);

	}

	// Add up the values within the slice.
	vst1q_u32(sumvals, resultslice);
	for (y = 0; y < (SHORTSIMDSIZE / 2); y++) {
		partialsum = partialsum + sumvals[y];
	}
...
float asum_float_simd(Py_ssize_t arraylen, float *data) { 
	...
	float32x4_t dataslice, resultslice;


	// Initialise the accumulator.
	resultslice = vdupq_n_f32(0.0);
	...
	for (x = 0; x < alignedlength; x += FLOATSIMDSIZE) {
		...
		// The actual SIMD operation. 
		resultslice = vaddq_f32(resultslice, dataslice);

	}

	// Add up the values within the slice.
	vst1q_f32(sumvals, resultslice);
